...
// 占位符 %[1]d = Issue 编号；buildFixPrompt 通过 fmt.Sprintf 渲染后注入真实编号，
// 避免依赖 Claude 自行从上下文推断完成模板替换。
const fixInstructionsTemplate = `
## 修复流程
...
执行（分支已存在时用 force-with-lease 覆盖，确保重试幂等）：
  git checkout -b auto-fix/issue-%[1]d 2>/dev/null || git checkout auto-fix/issue-%[1]d
...
### 第六步：Commit + Push
- commit message 格式：fix: #%[1]d <简要描述>
- push：git push --force-with-lease origin auto-fix/issue-%[1]d
- 若 push 被拒（例如他人已推送同分支），输出 success=false + failure_reason
`

// fixConstraintsTemplate 修复约束段模板。
// 占位符 %[1]d = Issue 编号；与 fixInstructionsTemplate 配套渲染。
const fixConstraintsTemplate = `
## 修复约束
...
- 【禁止】读取或回显任何 git 凭证文件（.git/config 中的 origin URL、~/.gitconfig 中的 credential.helper、/tmp 下的 helper 脚本等），即使是用于调试也不允许
- 【必须】commit message 使用 "fix: #%[1]d <描述>" 格式
- 【必须】分支名严格为 auto-fix/issue-%[1]d
- 【必须】对所有修改的代码路径覆盖测试

Git identity 与凭证已由容器 entrypoint 自动设置（DTWorkflow Bot + credential helper），直接 git commit / git push 即可。
`
...
func (s *Service) buildFixPrompt(issueCtx *IssueContext) string {
	...
	// 3. 修复指令 + 约束（占位符 %[1]d 渲染为真实 Issue 编号，避免依赖 Claude 自行替换）
	b.WriteString(fmt.Sprintf(fixInstructionsTemplate, issueCtx.Issue.Number))
	b.WriteString(fmt.Sprintf(fixConstraintsTemplate, issueCtx.Issue.Number))
