...
**目标**：提供 E2E 测试的执行、报告与失败分析能力，形成完整的自动化质量闭环

**依赖**：Phase 1（基础设施）+ Phase 3（Issue 自动创建 + auto-fix 能力）+ Phase 4（镜像策略）

### 关键设计决策

> 以下决策在 2026-05-08 brainstorming 中确认，指导后续设计与实施。

1. **职责分离（人控制"测什么"，平台负责"跑、报、修"）**：测试用例生成、Playwright/API 脚本编写、目录组织均为开发者本地行为（可借助 AI 辅助），不纳入 DTWorkflow 自动化管线。DTWorkflow 负责执行、报告、失败分析。这与 Phase 2-4 中 AI 全自主生成内容的模式有本质区别——E2E 测试的场景定义需要业务知识，AI 无法独立判断"该测什么"。
2. **自包含用例模型**：一个测试用例 = `case.yaml`（执行清单/manifest）+ 一组脚本（数据准备 SQL/JS + Playwright UI 测试 + API 调用测试 + 清理脚本）。每个用例自包含 setup → test → teardown 全流程，可在任意干净测试环境运行，不依赖外部预置数据。
3. **YAML 作为执行清单**：`case.yaml` 既是人可读的用例文档，也是 DTWorkflow 的执行契约。Claude 在容器内读取 `case.yaml` 来编排执行步骤。模块级可选 `suite.yaml` 声明子用例执行顺序和公共配置。
4. **Claude Code CLI 驱动执行**：与 Phase 2-4 保持架构一致，容器内由 Claude 读取 manifest 并编排执行（运行 SQL、Node 脚本、Playwright 命令等）。接受一定不确定性以换取灵活性——Claude 能理解不同脚本类型、处理意外情况、收集丰富的失败上下文。
5. **模块 = 容器 = 任务**：模块间通过 Worker 池并行（多容器），模块内 Claude 串行执行多个用例。复用 M4.2.1 的模块拆分模式。全量执行时扫描 `e2e/` 下的模块目录，每个模块创建独立任务并行调度。
6. **失败分析 + Phase 3 复用**：AI 分析失败原因，分类为真 bug / 脚本过时 / 环境问题。真 bug 和脚本过时均创建 Issue（附截图、错误栈、DOM 快照等上下文），脚本过时的 Issue 走 Phase 3 auto-fix 链路尝试自动修复。不在执行容器内做实时脚本修复。
7. **脚本维护策略（YAML 为 source of truth）**：开发者维护 YAML 用例和脚本，DTWorkflow 不做脚本自动生成或翻译。当 UI 变更导致脚本失效时，Phase 3 auto-fix 基于 YAML 用例意图 + 当前页面状态修复脚本。人永远只需编辑 YAML 和脚本，不需要理解 DTWorkflow 内部机制。
8. **两轮迭代**：第一轮（M5.1-M5.3）手动触发完整链路，验证执行 + 报告 + 失败分析；第二轮（M5.4-M5.5）回归自动化 + 探索性测试（可选扩展）。

### 不做什么

- **不做测试用例自动生成**：场景定义需要业务知识，属于开发者职责
- **不做 YAML → 脚本翻译**：开发者本地用 AI 辅助完成，不走 DTWorkflow 管线
- **不做测试环境部署/管理**：对接团队现有部署流程，仅做可用性检测

### 已有基础设施（Phase 1-4 已搭建）

以下能力在前序 Phase 中已实现，Phase 5 直接复用：

- **容器池 + 任务队列**：Worker 池管理、并发调度、任务持久化（Phase 1）
- **Service/Execute 模式**：Phase 2-4 统一的执行编排架构
- **模块拆分 + 并行执行**：M4.2.1 的 `ScanRepoModules` + 多任务拆分模式
- **通知框架**：飞书 + Gitea 通知（Phase 2 M2.6）
- **Issue 创建 + auto-fix**：Phase 3 的 Issue 自动修复能力
- **REST API + dtw 瘦客户端**：M3.3.1 的 API 层 + CLI 触发模式

**待新建**：
- E2E 专用 Docker 镜像（在现有镜像基础上叠加 Playwright + Chromium 浏览器）
- `internal/e2e` 包（Service/Execute 模式）

### 里程碑

---

#### 第一轮：手动触发 E2E 执行（完整链路）

> 通过 CLI / API 手动触发指定仓库的 E2E 测试套件。容器使用 E2E 镜像（含浏览器），Claude 按 case.yaml 编排执行，结果回写报告 + 创建 Issue。

##### M5.1 E2E 基础设施与执行引擎
- [ ] E2E Docker 镜像构建（现有 worker 镜像 + Playwright + Chromium，三级镜像策略）
- [ ] 新建 `internal/e2e` 包（Service/Execute 模式，仿 `internal/review` / `internal/test`）
- [ ] `TaskTypeRunE2E` 任务类型定义 + asynq 路由注册
- [ ] 测试环境对接：环境配置管理（测试 URL、数据库连接、账号等）、部署状态检测（确认环境可用后再执行）
- [ ] `case.yaml` / `suite.yaml` schema 定义（执行清单格式）
- [ ] E2E Prompt 设计：指导 Claude 读取 manifest、按阶段执行脚本（setup → test → teardown）、收集截图和错误上下文
- [ ] E2E 输出 JSON schema（`E2EOutput`：用例级 pass/fail/error + 截图路径 + 错误详情）
- [ ] 手动触发入口：REST API（`POST /api/v1/repos/{owner}/{repo}/e2e`）+ 服务端 CLI（`dtworkflow e2e run`）+ 瘦客户端（`dtw e2e run --repo X [--suite Y]`）
...
- [ ] 探索报告：发现的问题 + 建议的测试场景
- [ ] 有价值的发现经人确认后可纳入 YAML 用例集（回归套件增长机制）

### 交付物

**第一轮**：
- E2E 测试执行引擎（`internal/e2e` 包）
- E2E Docker 镜像（Playwright + Chromium）
- `case.yaml` / `suite.yaml` schema 定义
- AI 失败分析 + Issue 自动创建
- 手动触发入口（CLI / API / dtw）
