...
- 查询必须走 SQLite 时间函数（如 `julianday(created_at)`），不能依赖不同时间字符串格式的字典序比较
...
**注意**：不要混用 `time.DateTime` 与 RFC3339Nano 字符串做字典序比较。对任意时分秒窗口，这种比较会静默误判；这里统一使用 `julianday(...)` 做时间比较，优先保证正确性。
...
```go
	...
	if dailyReportHandler != nil {
		...
		entryID, schedErr := reportScheduler.Register(drCfg.Cron, asynq.NewTask(
			queue.AsynqTypeGenDailyReport,
			nil,
