...
评估了两种 DevFlow 与 DTWorkflow 的集成方案，最终选择方案 B。方案 A 作为备选保留，若未来需要将 DTWorkflow 提供给外部系统或跨网络部署，可回退到方案 A。

### 方案 A：松耦合 API 集成（备选）

```
                   ┌──────────────────────────────────┐
                   │      DevFlow (独立仓库 monorepo)   │
                   │                                    │
                   │  Vue 3 SPA ←──→ Go API Server     │
                   │       ↓              ↓             │
                   │  PostgreSQL(独立)  Redis (共享)     │
                   │       ↓                            │
                   │  内置网关 (Agent Router)             │
                   └───────┬──────────┬─────────────────┘
                           │          │
...
**特点**：
- DevFlow 和 DTWorkflow 各自拥有独立的 PostgreSQL 数据库（或同一 PG 实例的不同 database）
- DevFlow **完全通过 DTWorkflow REST API** 获取任务数据、触发操作
- 两个项目的数据库 schema 完全解耦，互不感知对方的表结构

**优势**：
- DTWorkflow 改动最小，仅需 SQLite→PG + 心跳注册，现有功能不受影响
- 两个项目可以不同团队独立开发，互不阻塞
- DTWorkflow 可被其他系统调用（通用性最好）
- 无 DB schema 耦合风险，DTWorkflow 表结构变更不影响 DevFlow

**劣势**：
- 报表和看板需要通过 API 聚合数据，无法直接 SQL JOIN，复杂度高
- 实时状态（任务进度、日志流）需要通过 WebSocket 中继，延迟略高
- 数据一致性依赖 API 同步，可能出现短暂不一致
- DevFlow 需要维护一份 DTWorkflow 数据的本地缓存或影子表

**适用场景**：
- DTWorkflow 需要提供给外部团队/系统使用
- 两个项目由不同团队维护，需要完全独立的发布节奏
- 部署跨多个网络区域，无法共享数据库
...
**特点**：
- DevFlow 和 DTWorkflow 共享同一个 PostgreSQL 实例（不同表集，同一 database）
- DevFlow 可直接读取 DTWorkflow 的任务/结果表（只读），写操作仍通过 API 或 asynq
- 实时事件通过 Redis pub/sub 直接订阅

**优势**：
- 数据一致性最好，无需同步机制
- 报表和看板直接 SQL JOIN，实现最简单
- 实时性高（直接读表 + Redis pub/sub，无中间层）
- 不需要维护数据缓存或影子表

**劣势**：
- 数据库 schema 耦合——DTWorkflow 表结构变更需要协调
- 要求所有服务在同一网络内可达共享 PG（当前场景满足）
- DevFlow 需遵守"只读 DTWorkflow 表"的约定

**选择理由**：
1. 同一团队维护两个项目，schema 协调成本远低于跨团队场景
2. 报表和任务看板是核心需求，直接 SQL 查询比 API 聚合简单一个数量级
3. 所有服务部署在同一内网，共享 PG 没有网络问题
4. 方案 B 是方案 A 的超集——DTWorkflow REST API 完整保留，需要时可平滑退回方案 A

**边界规则**：
- DevFlow 只写 `workspaces` / `members` / `repositories` / `repo_configs` / `chat_*` / `audit_log` 等 DevFlow 自有表
- DTWorkflow 只写 `tasks` / `review_results` / `test_gen_results` / `e2e_results` / `code_from_doc_results` / `iteration_*` 等 DTWorkflow 自有表
- 跨项目交互仅两种方式：DevFlow 只读 DTWorkflow 表 + Redis 事件订阅
...
### 4.1 Agent Router（DTWorkflow 实例管理）
...
| D3 | 集成方式 | 方案 B 共享 PostgreSQL（DevFlow 只读 DTWorkflow 表）；方案 A 松耦合 API 集成作为备选 | 报表 + 看板直接 SQL，无需数据同步；若需跨网络部署可回退方案 A |
